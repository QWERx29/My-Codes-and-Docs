...
struct node
{
    ...
    inline void init(int l, int v)
    {
        len = l;
        if (v)
            sum = pre1 = suf1 = ans1 = l, pre0 = suf0 = ans0 = 0;
        else
            sum = pre1 = suf1 = ans1 = 0, pre0 = suf0 = ans0 = l;
    }
} dat[N << 2];
...
{
    if (v < 2)
        dat[pos].init(Len, v), tag[pos] = v;
    else
    {
        dat[pos].flip();
        if (tag[pos] <= 1 && tag[pos] >= 0)
            tag[pos] ^= 1;
        else
            tag[pos] = 1 - tag[pos];
    }
...
int main()
{
    ...
    for (int i = 1; i <= q; i++)
    {
        ...
        if (op <= 2)
            update(Root, l, r, op);
        if (op == 3)
            cout << qry(Root, l, r).sum << endl;
        if (op == 4)
            cout << qry(Root, l, r).ans1 << endl;
    }
    return 0;
}