...
struct node{
    ...
    inline node init(int l,int v)
    {
        node res;
        res.len=v;
        if(l==1) res.pre=res.suf=res.ans=v;
        else res.pre=res.suf=res.ans=0;
        return res;
    }
}dat[N<<2];
...
{
    if(l==r) return l;
    if(dat[pos].ans<p) return 0;
    psd(This);
    if(dat[Lpos].ans>=p) return qry(Lson,p);
    if(dat[Lpos].suf+dat[Rpos].pre>=p) return Mid-dat[Lpos].suf+1;
    return qry(Rson,p);
}
inline void bld(int pos,int l,int r)
{
    tag[pos]=-1;
    if(l==r) return dat[pos]=dat[pos].init(1,1),void();
...
int main()
{
    ...
    for(int i=1;i<=q;i++)
    {
        ...
        if(op==1) 
        {
            ...
            x=qry(Root,d);
            cout<<x<<endl;
            if(x) update(Root,x,min(x+d-1,n),0);
        }
        if(op==2)
        {
            cin>>x>>d;
